...
namespace OpenPOS.CCO116Interop
{
    ...
    [ServiceObject(DeviceType.ToneIndicator, "OpenPOS 1.16 SoundPlayer", "OPOS SoundPlayer CCO Interop", 1, 16)]
    public class OpenPOSSoundPlayer : ToneIndicator, ISoundPlayer116, ILegacyControlObject, IDisposable
    {
        private OpenPOS.Devices.OPOSSoundPlayer _cco = null;
        private const string _oposDeviceClass = "SoundPlayer";
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_disposedValue)
            {
                ...
                if (_cco != null)
                {
                    _cco.DirectIOEvent -= (_IOPOSSoundPlayerEvents_DirectIOEventEventHandler)_cco_DirectIOEvent;
                    _cco.ErrorEvent -= (_IOPOSSoundPlayerEvents_ErrorEventEventHandler)_cco_ErrorEvent;
                    _cco.OutputCompleteEvent -= (_IOPOSSoundPlayerEvents_OutputCompleteEventEventHandler)_cco_OutputCompleteEvent;
                    _cco.StatusUpdateEvent -= (_IOPOSSoundPlayerEvents_StatusUpdateEventEventHandler)_cco_StatusUpdateEvent;
                    _cco = null;
                }
        ...
        public override void Open()
        {
            ...
            if (_cco == null)
            {
                try
                {
                    ...
                    _cco = new OpenPOS.Devices.OPOSSoundPlayer();

                    // Register event handler
                    _cco.DirectIOEvent += new _IOPOSSoundPlayerEvents_DirectIOEventEventHandler(_cco_DirectIOEvent);
                    _cco.ErrorEvent += new _IOPOSSoundPlayerEvents_ErrorEventEventHandler(_cco_ErrorEvent);
                    _cco.OutputCompleteEvent += new _IOPOSSoundPlayerEvents_OutputCompleteEventEventHandler(_cco_OutputCompleteEvent);
                    _cco.StatusUpdateEvent += new _IOPOSSoundPlayerEvents_StatusUpdateEventEventHandler(_cco_StatusUpdateEvent);
                }
                catch
                {
        ...
        #region SoundPlayer Specific Properties
        ...
        public bool CapMultiPlay
        {
            get { return _cco.CapMultiPlay; }
        }

        public string[] CapSoundTypeList
        {
            get { return _cco.CapSoundTypeList.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries); }
        }
        ...
        {
            get { return _cco.DeviceSoundList.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries); }
        }
        ...
        #endregion SoundPlayer Specific Properties
        ...
        #region SoundPlayer Specific Methods

        public void PlaySound(string fileName, bool loop)
        {
            VerifyResult(_cco.PlaySound(fileName, loop));
        }

        public void StopSound(int outputId)
        {
            VerifyResult(_cco.StopSound(outputId));
        }

        #endregion SoundPlayer Specific Methods
    }
}